...
export const UnifiedChat: React.FC = () => {
  const { chatMessages, sendChatMessage, connected } = useLiveKit();
    ...
    <div className="bg-gray-800 rounded-lg p-4">
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-400">
          {connected ? 'Connected — messages sync via LiveKit' : 'Not connected to LiveKit'}
        </p>
      </div>
      ...
      <form onSubmit={handleSendMessage} className="space-y-2">
        <textarea
          ...
          rows={2}
          placeholder={connected ? 'Type a message...' : 'Connect to LiveKit to chat'}
          disabled={!connected}
        />
        <button
          type="submit"
